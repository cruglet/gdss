...
func run(t: TC) -> void:
	...
	var label_handler: GdssPropHandler = GdssNodeHandler.get_primary_handler(label)
	GDSS.set_override_text(label, "transform_scale_y: 3")
	t.check_eq(label_handler._get_val("transform_scale"), Vector2(1.0, 3.0), "vector2 component override patches onto the default")
	GDSS.set_override_text(label, "transform_scale: 0.5")
	t.check_eq(label_handler._get_val("transform_scale"), Vector2(0.5, 0.5), "single-value vector2 override splats")
	GDSS.clear_overrides(label)
