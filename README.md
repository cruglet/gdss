...
- `border_color` and `shadow` are per-side too. Name a side to override just that one, like `border_color_top: RED` or `shadow_bottom: 8`; sides you leave out keep the shorthand's value, and a shorthand written afterwards resets all four. Per-side border colors meet on the corner diagonal, and a shadow tapers out around corners where the neighbouring side is 0.
...
- Methods compute values, such as `linear_gradient`, `radial_gradient`, `mix`, `lighten`, `darken`, `saturate`, `desaturate`, `grayscale`, `invert`, `complement`, `contrast`, `hsv`, `hsv_shift`, `alpha`, `rgba`, `clamp`, `blur`, and `texture`. Use `pass` for an optional argument to fall back to its default, like `linear_gradient($a, $b, pass, 0.2, 0.8)`. Methods can be nested, like `linear_gradient(alpha(BLACK, 0.2), TRANSPARENT_BLACK)`.
- `blur(strength, color, color_opacity)` frosts the panel's backdrop - it blurs whatever is behind it and tints it, like `bg_color: blur(2, BLACK, 0.2)`. `liquid_blur(strength, color, color_opacity, refraction, highlight)` builds on it with an iOS/macOS-style refraction that bends and magnifies the backdrop through the edges, plus a swept edge glow, like `bg_color: liquid_blur(3, WHITE, 0.06)`. Both also work on `border_color` to frost just the border. The liquid-glass refraction is ported from [OverShifted/LiquidGlass](https://github.com/OverShifted/LiquidGlass) (MIT).
